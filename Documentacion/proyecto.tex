...
\begin{titlepage}
    ...
    \normalsize
    Documento de Arquitectura y Especificación de Proyecto \\
    \today
...
En la actualidad, los condominios y complejos residenciales enfrentan grandes desafíos en cuanto a seguridad, gestión comunitaria y control de accesos. El problema principal radica en la dependencia exclusiva de la vigilancia humana y sistemas análogos o desconectados entre sí. Esta dependencia genera puntos ciegos, fatiga visual en los conserjes, y tiempos de reacción extremadamente lentos ante posibles intrusiones o comportamientos anómalos. 

Ejemplos comunes de estas anomalías incluyen la presencia de vehículos no autorizados en estacionamientos, el tránsito de mascotas en áreas restringidas (como piscinas o zonas de juegos infantiles exclusivas), o el acceso de personas ajenas a la comunidad en horarios no permitidos.

Este problema afecta directamente a múltiples actores dentro de la comunidad:
\begin{itemize}
...
La solución propuesta es el \textbf{Condominio Smart System}, una plataforma integral de última generación que combina el Internet de las Cosas (IoT) con Inteligencia Artificial (IA) para crear un entorno residencial seguro, autónomo y proactivo.

El sistema funcionará mediante la integración de hardware de bajo costo (microcontroladores ESP32 y sensores físicos) instalados en los pórticos, barreras vehiculares y perímetros vulnerables, los cuales detectarán movimiento y proximidad de manera determinista. De manera simultánea y complementaria, un módulo de Visión por Computadora (IA) analizará los flujos de video continuo de las cámaras de seguridad IP ya existentes en el recinto. Toda la información recabada, tanto del mundo físico (IoT) como del análisis de video (IA), será enviada en tiempo real mediante el protocolo MQTT hacia un ecosistema de microservicios central (Backend).

Cuando el sistema detecte una anomalía, el servidor procesará el evento a través de su motor de reglas y enviará comandos automáticos de regreso al hardware para activar alarmas físicas (como buzzers disuasorios y balizas luminosas). En paralelo, notificará a través de una aplicación web al personal de recepción, guardias de seguridad y a los residentes afectados.

El valor agregado para el usuario y el administrador es inmenso. El sistema garantiza una vigilancia activa 24/7 sin interrupciones, automatiza las alertas y disuade a los intrusos de forma inmediata. A diferencia de un sistema de cámaras tradicional (donde el video solo sirve como evidencia forense posterior al delito), el \textit{Condominio Smart System} actúa de forma preventiva y proactiva, detectando y mitigando la incidencia en el instante mismo en que ocurre.

\section{Definición de Perfiles del Sistema}
Para garantizar la escalabilidad, la privacidad de los datos y el principio de mínimo privilegio, el sistema segmenta las funcionalidades en cuatro roles bien definidos, cada uno con interfaces y alcances específicos dentro de la plataforma web.
...
\subsection{Perfil de Usuario (RESIDENTE)}
Diseñado para los habitantes del condominio, enfocado en brindar transparencia, comunicación y tranquilidad.
\begin{itemize}
...
\begin{itemize}
    \item \textbf{Microcontrolador ESP32:} Funciona como el nodo principal de procesamiento en el borde (Edge). Gracias a su módulo Wi-Fi integrado, se conecta a la red del condominio y establece una comunicación bidireccional mediante el protocolo ligero MQTT. Es responsable de leer los sensores y accionar los actuadores en cuestión de milisegundos.
    ...
    \item \textbf{Sensor Magnético (Reed Switch):} Instalado en puertas de emergencia. Envía una alerta si la puerta es abierta de forma indebida.
...
% Lines
\path [line, <->] (iot) -- node[above, font=\scriptsize] {Pub/Sub} (mqtt);
\path [line, ->] (ia) -- node[right, font=\scriptsize] {Eventos JSON} (mqtt);
\path [line, <->] (mqtt) -- node[above, font=\scriptsize] {Integración} (monitor);
\path [line, ->] (mqtt) -- (notif);

\path [line, <->] (monitor) -- (db);
\path [line, <->] (auth) -- (db);

\path [line, <->] (nginx) -- (auth);
\path [line, <->] (nginx) -- (monitor);
\path [line, <->] (frontend) -- node[right, font=\scriptsize] {HTTP/REST \& WS} (nginx);
